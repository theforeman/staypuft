...
tests['simple'] = lambda do |compute_resource, hostgroup, _hg|
  ...
  result = ForemanTasks.trigger Actions::Staypuft::Host::Create, "rhel#{rand 1000}",
                                hostgroup, compute_resource
  raise result.error unless result.triggered?
  result.finished.value!

  result = ForemanTasks.trigger Actions::Staypuft::Hostgroup::OrderedDeploy,
                                [hostgroup],
                                hostgroup.hosts.select { |h| !h.installed_at }
  raise result.error unless result.triggered?
  result.finished.value!
end
...
tests['complex'] = lambda do |compute_resource, hostgroup1, hostgroup2|
  ...
  results = [ForemanTasks.trigger(Actions::Staypuft::Host::Create, "rhel#{rand 1000}",
                                  hostgroup1, compute_resource, start: false),
             ForemanTasks.trigger(Actions::Staypuft::Host::Create, "rhel#{rand 1000}",
                                  hostgroup2, compute_resource, start: false),
             ForemanTasks.trigger(Actions::Staypuft::Host::Create, "rhel#{rand 1000}",
                                  hostgroup2, compute_resource, start: false)]
  ...
  result = ForemanTasks.trigger Actions::Staypuft::Hostgroup::OrderedDeploy,
                                hostgroups,
                                hostgroups.inject([]) { |a, hg| a + hg.hosts }.
                                    select { |h| !h.installed_at }
  raise result.error unless result.triggered?
  result.finished.value!
end
